...
import h5py
import numpy as np
#from models.unet.functions.helpers import *
...
def combine_coil_rss(multi_images, orig_shape, csm=None):
    ...
    if imgs_out.shape[0] > 320:
       s = orig_shape[-2:]
       imgs_out = crop_center(imgs_out,320,320)
    return imgs_out
'''
...
def rss_recon2(kspace):
    # multi_imgs = ifft2(kspace)
    imgs = []
    for img in kspace.detach().cpu():
        imgs += [ img[:,:,0].numpy() , img[:,:,1].numpy() ]
    chn_imgs = channels2imgs(np.array(imgs))
    combined = root_sum_of_squares(torch.from_numpy(chn_imgs))
    cropped = crop_center(combined, 320,320)
...
def data_consistency(net, ni, mask1d, slice_ksp_torchtensor1, orig_shape, csm=None):
    ...
    Fimg = mri_fft2(fimg, csm)  # dim: (1,num_slices,x,y,2)
    # ksp has dim: (num_slices,x,y)
    meas = slice_ksp_torchtensor1.data.cpu()  # dim: (1,num_slices,x,y,2)
    csm = csm.data.cpu() if csm is not None else None

    mask = mask1d[0].data.cpu() #torch.from_numpy(np.array(mask1d, dtype=np.uint8))
    ksp_dc = Fimg.clone()
    ksp_dc = ksp_dc.detach().cpu()
#    print(f"ksp_dc: {ksp_dc.device}, meas: {meas.device}, mask: {mask.device}")
    ...
    img_dc = ifft2(ksp_dc, csm)[0] if csm is None else ifft2(ksp_dc, csm)
    print(f"img_dc: {img_dc.shape}")
    out = []
    ...
    print(f"after channels2imgs: {par_out_imgs.shape}")
    prec = root_sum_of_squares(torch.from_numpy(par_out_imgs)).numpy()

    prec = crop_center(prec, 320, 320)
    return prec

def get_scale_factor(net, ni, masked_kspace, csm=None,  batch_size=1):
    ...
    orig_tt = ifft2(masked_kspace, csm)  # Apply Inverse Fourier Transform to get the complex image
...
#    slice_tt = torch.from_numpy(slice).type(dtype)
    ...
    for k in range(5):
        ...
        if k == 0:
            slice_scale = slice * torch.norm(output.detach()) / torch.norm(input.detach())
        else:
            slice_scale *= torch.norm(output.detach()) / torch.norm(input.detach())
        input = torch.moveaxis( rss_torch(ifft2(slice_scale))[None,:] , -1, 1 )

    scale = (torch.norm(slice_scale) / torch.norm(slice)).data.cpu().numpy()[np.newaxis][0]
    return scale
...
class MaskFunc_highfreq:
    ...
    def __call__(self, shape, seed=None):
        ...
        #mask[pad:pad + num_low_freqs] = True
        emp = num_cols // 2 - 160
        mask[emp:emp+num_low_freqs] = True
        mask[-(emp+num_low_freqs):] = True
        print(f"num of preserved frequencies: {num_low_freqs}")
...
def crop_center(img,cropy,cropx):
    assert 0 < cropy <= img.shape[-2]
    assert 0 < cropx <= img.shape[-1]
    if img.shape[-2] == cropy and img.shape[-1] == cropx:
       return img
...
def complex_crop_center(img, cropx, cropy):
    """
    the last dimension is 2
    """
    assert 0 < cropy <= img.shape[-3]
    assert 0 < cropx <= img.shape[-2]
    y,x = img.shape[-3], img.shape[-2]
    startx = x//2-(cropx//2)
    starty = y//2-(cropy//2)
    return img[...,starty:starty+cropy,startx:startx+cropx]

def get_mask(slice_ksp_torchtensor, slice_ksp, factor=4, cent=0.07, low_freq_only_mask=False, inverse_mask=False, seed=6100):
    """
    ...
    """
    if inverse_mask:
       print("Sampled high frequencies only.")
       masked_kspace, mask = apply_mask(slice_ksp_torchtensor,
                                   mask_func=MaskFunc_highfreq(center_fractions=[cent], accelerations=[factor]), seed=seed)
    elif low_freq_only_mask:
       num_cols = slice_ksp_torchtensor.shape[-2]
       mask = np.zeros(num_cols) # num of columns
       num_low_freqs = int(round(num_cols * cent)) 
       half_low = num_low_freqs // 2
       mask[num_cols//2-half_low:num_cols//2+half_low] = True
       print(f"Sampled {half_low*2} low frequencies only.")
#       mask = 1 - mask

       mask_shape = [1 for _ in slice_ksp_torchtensor.shape]
       mask_shape[-2] = num_cols
       mask = torch.from_numpy(mask.reshape(*mask_shape).astype(np.float32))

       masked_kspace, mask = apply_mask(slice_ksp_torchtensor, mask=mask)
    else:
       print(f"Normal under-sampling.")
       masked_kspace, mask = apply_mask(slice_ksp_torchtensor,
                                   mask_func=MaskFunc(center_fractions=[cent], accelerations=[factor]), seed=seed)
    mask1d = mask[0, 0, :, 0]
...
def mri_fft2(data, csm=None):
    ...
    if csm is not None:
       ...
       data = torch.view_as_complex(data).expand_as(csm)
       scaled = data * csm
       data = torch.stack((scaled.real, scaled.imag), -1)
...
def ifft2(data, csm=None):
    ...
    if csm is not None:
       ...
       csm = torch.view_as_complex(csm)
       data = torch.view_as_complex(data).expand_as(csm)
       scaled = (data * torch.conj(csm)).sum(1)
       print(f"csm scaled data during ifft: {scaled.shape}")
       data = torch.stack((scaled.real, scaled.imag), -1)
       print(f"{data.shape}")
