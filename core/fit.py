...
from losses import loss_func, tv_loss
...
def fit(args,
    ...
    PSNR_list, SSIM_list, Loss_list, Masked_PSNR_list, Masked_SSIM_list, Unmasked_PSNR_list, Unmasked_SSIM_list = [], [], [], [], [], [], []
    ...
    for i in tqdm(range(args.num_iters)):
        ...
        if args.print_all_images_forward:
           down_out, unmasked_img, masked_img, unmasked_gt, masked_gt = apply_f(out, mask, fully_meas=fully_meas, downsampler=downsampler)
        else:
           down_out = apply_f(out, mask, downsampler)

        total_loss = loss_func(down_out, img_noisy_var)
        ...
        if (i+1) % args.iters_cal_acc == 0:
            ...
            if args.task == 'mri_knee' or args.task == 'mri_brain':
                ...
                if (i+1) % args.iters_print_acc == 0:
                    ...
                    print('Iteration %05d  baseline_psnr: %f  baseline_ssim: %f  PSRN_gt: %f SSIM_gt: %f  (low-freq)PSNR_gt: %f (low-freq)SSIM_gt: %f (high-freq)PSNR_gt: %f (high-freq)SSIM_gt: %f' % (
                  i,  metrics_baseline['psnr'], metrics_baseline['ssim'], metrics['psnr'], metrics['ssim'], unmasked_metrics['psnr'], unmasked_metrics['ssim'], masked_metrics['psnr'], masked_metrics['ssim']))
                ...
                i,  noisy_baseline, psnr_noisy, metrics_plain['psnr'], metrics['psnr']))
                    # store the image snapshots
                    save_image(f"{snapshots_save_path}/{i+1}_snapshot.png", out)
        ...
        if (i+1) % 10 == 0 and args.num_power_iterations: 
    ...
    return out_avg, best_net, metrics['psnr'], metrics['ssim'], norms_list, singular_list
