...
def main(args):
    ...
    for iter, sample in enumerate(train_data):
        ...
        if args.task == 'sr':
           downsampler = Downsampler(n_planes=3, factor=args.sr_factor, kernel_type=args.sr_kernel_type, phase=0.5, preserve_size=True).type(dtype)
        else:
           downsampler = None
        ...
        if args.model_type == 'PruneDIP':
           rec, layerID  = trained_net(ni, args.min_tau, noisy_target, orig, training=False, save_path=args.exp_name)
           rec = rec[0].data.cpu().numpy()
           print(f"Best depth: the {layerID+1} layer ")
        else:
           layerID = None
        ...
        noisy_metrics = eval_general(torch_to_np(orig), torch_to_np(noisy_target))
...
#            print(compare_psnr(torch_to_np(orig).transpose(1,2,0), torch_to_np(rec).transpose(1,2,0))
        ...
        if layerID is not None:
           info = {'noisy_target': noisy_target[0].data.cpu().numpy(), 'rec': out_avg[0], 'orig': orig[0].data.cpu().numpy(), 'p_score': metrics['psnr'], 's_score': metrics['ssim'], 'noisy_p_score': noisy_metrics['psnr'], 'time': 1.0,
               'noisy_s_score': noisy_metrics['ssim'], 'filename': filename_full, 'savename': (f"{args.exp_name}/results"), 'depth': layerID}
        else:
           info = {'noisy_target': noisy_target[0].data.cpu().numpy(), 'rec': out_avg[0], 'orig': orig[0].data.cpu().numpy(), 'p_score': psnr_score, 's_score': ssim_score, 'noisy_p_score': noisy_metrics['psnr'], 'time': 1.0,
               'noisy_s_score': noisy_metrics['ssim'], 'filename': filename_full, 'savename': (f"{args.exp_name}/results")}

        visualize(info)
        save_scores(args, info)

        if rec2_available:
           info2 = {'noisy_target': noisy_target[0].data.cpu().numpy(), 'rec': rec2, 'orig': orig[0].data.cpu().numpy(), 'p_score': metrics2['psnr'], 's_score': metrics2['ssim'], 'noisy_p_score': noisy_metrics['psnr'], 'time': 1.0,
        ...
        #if len(spectral_norms) > 0 and args.num_iters == 3000:
        #  plot_multiple_spectral_norms(spectral_norms, f"{args.exp_name}/spectral_norm_all_layers")
        #  plot_singular_values_all_iters(singular_list, f"{args.exp_name}/singular_values_all_iters")
