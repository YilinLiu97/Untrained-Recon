...
from unet3d_utils.buildingblocks import DoubleConv, ResNetBlock, ResNetBlockSE, \
    create_decoders, create_encoders
from unet3d_utils.utils import get_class, number_of_features_per_level
...
class UNet3D(AbstractUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=False, f_maps=96, layer_order='gcr',
                 num_groups=8, num_levels=5, is_segmentation=False, conv_padding=1,
                 conv_upscale=2, upsample='nearest', dropout_prob=0.0, **kwargs):
...
class ResidualUNetSE3D(AbstractUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=False, f_maps=64, layer_order='gcr',
                 num_groups=8, num_levels=5, is_segmentation=False, conv_padding=1,
                 conv_upscale=2, upsample='default', dropout_prob=0.0, **kwargs):
...
model=UNet3D(192,1, final_sigmoid=False, f_maps=128)
x = torch.rand(1,192,256,256,96)
out=model(x)
print(out.shape)
